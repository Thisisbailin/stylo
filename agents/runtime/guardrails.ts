    ...
    return [
      defineToolInputGuardrail({
        ...
        run: async ({ toolCall }) => {
          ...
          if (!entity || !action) {
            return ToolGuardrailFunctionOutputFactory.rejectContent("edit_script_resource requires entity and action.");
          }
          if (!["archive"].includes(entity)) {
            return ToolGuardrailFunctionOutputFactory.rejectContent(
              "edit_script_resource only supports archive. Folder nodes are generated by foundation operations.",
              { entity }
            );
          }
          if (!["create", "update"].includes(action)) {
            return ToolGuardrailFunctionOutputFactory.rejectContent("Script resources only support create or update.", {
              entity,
              action,
            });
          }
          if (action === "update") {
            ...
            if (!nodeId && !nodeRef && !documentId) {
              return ToolGuardrailFunctionOutputFactory.rejectContent(
                "Updating a script resource requires node_id, node_ref, or document_id.",
                { entity, action }
              );
            }
          }
      ...
      defineToolOutputGuardrail({
        ...
        run: async ({ output }) => {
          ...
          if (
            !result ||
            result.updated !== true ||
            (target !== "script:archive" && target !== "script:folder") ||
            artifactKind !== "node"
          ) {
        ...
        run: async ({ output }) => {
          ...
          const artifact =
            result?.artifact && typeof result.artifact === "object"
              ...
              : null;
          const artifactKind = typeof artifact?.kind === "string" ? artifact.kind : "";
          if (result?.updated === false && result?.skipped === true) {
