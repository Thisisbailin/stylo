...
Stylo 的一次运行必须且只能属于一个 Flow Project。项目切换不得复用另一项目的对话、模型记忆、工具活动、审批状态、角色素材或异步执行结果。

## 核心不变量

1. `projectId` 是一次 Stylo run 的不可变作用域。
2. `sessionId` 必须包含项目作用域，格式为 `stylo:{encodedProjectId}:{encodedConversationId}`。
...
| 状态 | 作用域 | 实现 |
| --- | --- | --- |
| 对话列表与消息 | 项目 | `stylo_conversations_v2:{projectId}` |
| Agent session / 压缩记忆 | 项目 + 对话 | 带项目名前缀的 `sessionId` 与 D1 `session_key` |
| 工具活动摘要 | 项目 | `stylo_agent_tool_activity_v2:{projectId}` |
...
4. 以新的 React `key` 重建 Stylo 和历史面板；旧组件卸载时 abort 当前请求。
5. 清空临时执行状态和待审批状态。
...
- 旧的全局 Stylo 对话只迁移到第一个项目一次，之后删除旧 key。
