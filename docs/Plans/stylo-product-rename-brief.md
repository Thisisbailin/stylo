...
The rename must not make existing local projects, Agent conversations, saved desktop state, exported project packages, or current Cloudflare deployments unreadable while the external GitHub and Cloudflare resources still use their former `qalam` identifiers.

## Out of scope

- Renaming the GitHub repository or Cloudflare Pages/D1 resources. The user will perform those external operations later.
...
- Existing Qalam browser storage, session identifiers, project packages, Electron bridge clients, environment variables, and current external deployment URLs remain readable through an explicit legacy compatibility layer.
- Desktop and web icon targets are generated from or reference the current Stylo artwork.

## Acceptance Criteria

1. Active user-facing product text contains no Qalam branding; the current GitHub/Cloudflare identifiers may appear only as documented legacy external coordinates.
...
6. `STYLO_*` and `VITE_STYLO_*` configuration names take precedence, with legacy `QALAM_*` fallbacks until external deployment is renamed.
...
- Do not access or modify external GitHub/Cloudflare state.
...
- Current remote URL, Pages project, D1 database, and repository still use `qalam`. Mitigation: isolate them as legacy external coordinates until the external rename happens.
...
- Cloudflare: keep existing deployed resource coordinates in compatibility configuration until the user renames them externally.
