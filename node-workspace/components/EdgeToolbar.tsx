...
export const EdgeToolbar: React.FC<Props> = ({ linkId }) => {
  const { links, removeLink, toggleLinkPause } = useNodeFlowStore();
  ...
  return (
    <div className="flex items-center gap-2 app-panel px-2 py-1 rounded">
      <button
        onClick={() => toggleLinkPause(linkId)}
      ...
      <button onClick={() => removeLink(linkId)} className="px-2 py-1 text-xs bg-red-600 text-white rounded">
        Delete
      </button>
    </div>
  );
