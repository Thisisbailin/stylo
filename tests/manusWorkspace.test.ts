...
test("Manus presents hidden floating tools, connected pages, and a LookBook identity rail", async () => {
  ...
  assert.match(chrome, /layout[\s\S]*className="screenplay-identity-dock__surface"/);
  ...
  assert.match(styles, /\.screenplay-header \{[\s\S]*display: none;/);
  ...
  assert.match(styles, /\.screenplay-workspace\.is-agent-open \.screenplay-document-stage\.is-vertical \{[\s\S]*padding-right: 0;/);
